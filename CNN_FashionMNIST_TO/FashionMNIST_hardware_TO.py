...
hardware_weight = torch.load('CNN_RRAM_hardware_weight.pt', map_location='cpu')
weight_list_one = []
weight_list_zero = []
...
class MaskConv(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        # Load hardware-specific weights. These weights are used to simulate hardware behavior in the neural network.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Pytorch CNN with Hardware TO")

    parser.add_argument('--batch-size', type=int, default=180, metavar='N',
    ...
    if args.save_model:
        torch.save(best_dict, "fashionmnist_hardware_TO_program_{}_".format(best) + timestamp_str + ".pt")
        shape_list = []
        ...
        with open("fashionmnist_hardware_TO_program_{}_".format(best) + timestamp_str + ".txt", 'w') as f:
            f.write('arch:\n')
            for s in shape_list:
                arch = str(s[0])
                for t in s[1:]:
