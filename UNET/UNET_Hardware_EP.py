...
def plot_weight(x, name, out_dir=None, aspect=1):
    ...
    plt.imshow(x, interpolation='nearest', cmap='inferno', aspect=aspect)
...
def inject_noise(w):
    ...
    out_n = torch.where(out_n <= 0.0, torch.tensor(1e-9).cuda(), out_n)
...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, mid_channels=None):
        ...
        self.conv1 = SupermaskConv(in_channels, mid_channels, kernel_size=3, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(mid_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = SupermaskConv(mid_channels, out_channels, kernel_size=3, padding=1, bias=False)
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.up = SupermaskConvTranspose(in_channels, in_channels // 2, kernel_size=2, stride=2)
...
class OutConv(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.out_conv = SupermaskConv(in_channels, num_classes, kernel_size=1, bias=False)
...
def main():
    ...
    # Training settings
    parser = argparse.ArgumentParser(description="Pytorch Unet with Hardware Edge_Popup")
    ...
    train_loader = torch.load('train_loader_6400.pt')
    test_loader = torch.load('val_loader_6400.pt')
    ...
    Set = np.concatenate((hardware_weight["set"][:, : half_size], hardware_weight["reset"][:, half_size:]), axis=-1)[
          :50].reshape([-1])
    Reset = np.concatenate((hardware_weight["set"][:, half_size:], hardware_weight["reset"][:, : half_size]), axis=-1)[
            :100].reshape([-1])
    hardware_weight = 0

    model_state_dict = torch.load('init_weight.pt')['net']
    ...
    for i in model_state_dict.keys():
        if "bn" not in i:
            if "score" not in i:
                size = model_state_dict[i].numel()
                one_weight_dict[i] = (Set[index: index + size] - Reset[index: index + size]).reshape(
                    [*model_state_dict[i].shape])
                zero_weight_dict[i] = (Reset[index: index + size] - Reset[index + 50: index + 50 + size]).reshape(
                    [*model_state_dict[i].shape])
                index += size
    model_state_dict = []
    hardware_weight = []
