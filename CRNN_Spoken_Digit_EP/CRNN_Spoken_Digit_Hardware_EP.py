...
def inject_noise(w):
    """
    Injects Gaussian noise into weights based on their magnitude.
    
    Args:
        w: A numpy array of weights.
    
    Returns:
        A numpy array of weights with injected noise.
    """
    # Ensure non-zero weight to avoid division by zero
    w = w + 1e-9
    # Compute standard deviation of noise based on weight magnitude
    sigma = p_noise / 100 * abs(w)
    # Inject Gaussian noise
    w_n = np.random.normal(loc=w, scale=sigma)
    # Clip noise-injected weights to within 2 standard deviations to reduce outliers
    p2s = w + 2 * sigma
    n2s = w - 2 * sigma
    w_n = np.where(w_n > p2s, p2s, w_n)
    w_n = np.where(w_n < n2s, n2s, w_n)
    # Ensure no weights are exactly zero after noise injection
    w_n = np.where(w_n == 0.0, 1e-9, w_n)

    return w_n

# Load pre-existing hardware weights
hardware_weight = torch.load('RRAM_hardware_weight_1200.pt', map_location='cpu')
# Deep copy hardware weights to modify with noise
hardware_weight_n = copy.deepcopy(hardware_weight)
# Inject noise into each weight tensor
...
for i in model_state_dict.keys():
    if "weight" in i:
        ...
        weight_list_one.append((hardware_weight["set"][:, index: index + size] - hardware_weight["set"][:, half_size + index: half_size + index + size]).reshape([hardware_weight["set"].shape[0]] + model_state_dict[i]))
        weight_list_zero.append((hardware_weight["reset"][:, index: index + size] - hardware_weight["set"][:, half_size + index: half_size + index + size]).reshape([hardware_weight["set"].shape[0]] + model_state_dict[i]))
        weight_list_one_n.append((hardware_weight_n["set"][:, index: index + size] - hardware_weight_n["set"][:, half_size + index: half_size + index + size]).reshape([hardware_weight_n["set"].shape[0]] + model_state_dict[i]))
        weight_list_zero_n.append((hardware_weight_n["reset"][:, index: index + size] - hardware_weight_n["set"][:, half_size + index: half_size + index + size]).reshape([hardware_weight_n["set"].shape[0]] + model_state_dict[i]))
        index += size
...
class GetSubnet(autograd.Function):
    ...
    def forward(ctx, scores, k, mask_old, score_old):
        """
        Forward pass for generating the supermask.
        Args:
            ctx: Context object to stash information for backward computation.
            scores: The scores for each weight based on which the supermask is computed.
            k: The sparsity level as a fraction representing the top k% weights to be active.
            mask_old: The previous mask used for limiting updates.
            score_old: The previous scores used for calculating deltas.
        
        Returns:
            A binary supermask tensor where the top k% of weights by score are set to 1, the rest are 0.
        """
    ...
    def backward(ctx, g):
        ...
        Args:
            g: Gradient of the loss with respect to the output of this layer.
        
        Returns:
            Tuple of gradients with respect to the inputs, with only the gradient g being propagated.
        """
    ...
    def forward(self, x, count, switch_count=None):
        ...
        if switch_count is not None:
            ...
            w = self.sign * torch.where(subnet != switch_count[0],
                                        self.hardware_weight_one_n[count] * subnet + self.hardware_weight_zero_n[count]
                                        * (1 - subnet),
                                        self.hardware_weight_one[count] * subnet + self.hardware_weight_zero[count]
                                        * (1 - subnet))
            x = F.conv2d(
...
class SupermaskRNN(nn.RNNBase):
    def __init__(self, *args, **kwargs):
        ...
        for i in range(len(weight_list_one)):
            if weight_list_one[i][0].shape == self.weight_ih_l0.data.shape:
                ...
                self.hardware_weight_one_n_ih = torch.from_numpy(weight_list_one_n[i].copy()).abs().float().cuda()
                self.hardware_weight_zero_n_ih = torch.from_numpy(weight_list_zero_n[i].copy()).abs().float().cuda()
                ...
                self.hardware_weight_one_n_hh = torch.from_numpy(weight_list_one_n[i].copy()).abs().float().cuda()
                self.hardware_weight_zero_n_hh = torch.from_numpy(weight_list_zero_n[i].copy()).abs().float().cuda()
            else:
                pass
    ...
    # Execution of the RNN step using either tanh or relu activation, as specified by the `mode` attribute.
        ...
        weight_ih_l0_subnet = GetSubnet.apply(self.ih_scores.abs(), args.sparsity, self.ih_subnet_old, self.ih_score_old.abs())
        weight_hh_l0_subnet = GetSubnet.apply(self.hh_scores.abs(), args.sparsity, self.hh_subnet_old, self.hh_score_old.abs())
        ...
        if switch_count_ih is not None:
            ...
            ih_n = self.hardware_weight_one_n_ih[count] * weight_ih_l0_subnet + self.hardware_weight_zero_n_ih[count] * (1 - weight_ih_l0_subnet)
            ih = self.hardware_weight_one_ih[count] * weight_ih_l0_subnet + self.hardware_weight_zero_ih[count] * (1 - weight_ih_l0_subnet)
            hh_n = self.hardware_weight_one_n_hh[count] * weight_hh_l0_subnet + self.hardware_weight_zero_n_hh[count] * (1 - weight_hh_l0_subnet)
            hh = self.hardware_weight_one_hh[count] * weight_hh_l0_subnet + self.hardware_weight_zero_hh[count] * (1 - weight_hh_l0_subnet)

            w_l = [
                    self.ih_sign * torch.where(weight_ih_l0_subnet != switch_count_ih[0], ih_n, ih) / self.ih_scale,

                    self.hh_sign * torch.where(weight_hh_l0_subnet != switch_count_hh[0], hh_n, hh) / self.hh_scale
                ]
            switch_count_ih[0] = weight_ih_l0_subnet.data.clone()
            switch_count_hh[0] = weight_hh_l0_subnet.data.clone()
        else:
            w_l = [
                self.ih_sign * (self.hardware_weight_one_ih[count] * weight_ih_l0_subnet + self.hardware_weight_zero_ih[count] * (1 - weight_ih_l0_subnet)) / self.ih_scale,

                self.hh_sign * (self.hardware_weight_one_hh[count] * weight_hh_l0_subnet + self.hardware_weight_zero_hh[count] * (1 - weight_hh_l0_subnet)) / self.hh_scale
            ]
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = SupermaskConv(model_state_dict['conv1_weight'][1], model_state_dict['conv1_weight'][0], model_state_dict['conv1_weight'][-2:], [2, 1], bias=False)
        self.c_bn1 = nn.BatchNorm2d(model_state_dict['conv1_weight'][0], affine=False)
        self.conv2 = SupermaskConv(model_state_dict['conv2_weight'][1], model_state_dict['conv2_weight'][0], model_state_dict['conv2_weight'][-2:], [2, 1], bias=False)
        self.c_bn2 = nn.BatchNorm2d(model_state_dict['conv2_weight'][0], affine=False)
        self.rnn = SupermaskRNN(mode='RNN_RELU', input_size=model_state_dict['rnn_ih_weight'][-1], hidden_size=model_state_dict['rnn_ih_weight'][0], num_layers=1, bias=False,
                                batch_first=False)
