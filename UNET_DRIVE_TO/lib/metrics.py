...
class Evaluate():
    ...
    def auc_roc(self, plot=False):
        ...
        if plot and self.save_path is not None:
            ...
            plt.title('ROC Curve', pad=20, fontdict=font, fontsize=44)
            plt.xlabel("FPR (False Positive Rate)", fontdict=font, labelpad=10)
            plt.ylabel("TPR (True Positive Rate)", fontdict=font, labelpad=10)
            plt.legend(loc="lower right")
            fig = plt.gcf()
            fig.set_size_inches(19.20, 10.80)
            plt.savefig(join(self.save_path, "ROC.pdf"), dpi=600, transparent=True)
            plt.close("all")
        return AUC_ROC
